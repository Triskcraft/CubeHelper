...
class CubeHelperOnMessage(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        if message.author.bot:
            ...
            await MaterialLists.updateListEmbed(message.reference.message_id, self.client)
            await message.delete()
            return
        ...
        if not isDev(message.author): return
            ...
            await message.channel.send(embed = Introduction.embed(self.client), view = Introduction.views())
            await message.delete()
